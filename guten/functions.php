 ...
 * @package Guten
 */
define( 'GUTEN_THEME_VERSION', '1.1.26' );
...
function guten_scripts() {
	...
	wp_enqueue_style( 'guten-font-awesome', get_template_directory_uri().'/includes/font-awesome/css/all.css', array(), '5.15.3' );
	...
	// if ( get_theme_mod( 'guten-plugin-buddypress', customizer_library_get_default( 'guten-plugin-buddypress' ) ) ) :
	// 	wp_enqueue_style( 'guten-buddypress-style', get_template_directory_uri()."/includes/css/buddypress.css", array(), GUTEN_THEME_VERSION );
	// endif;
 ...
 */
function guten_custom_footer_scripts() {
	// The following is minified via 'terser --compress --mangle -- js/skip-link-focus-fix.js' ?>
	<script>
	/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a|select|input|button|textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);
	</script><?php
}
add_action( 'wp_print_footer_scripts', 'guten_custom_footer_scripts' );
...
function guten_register_required_plugins() {
	$plugins = array(
		...
		array(
			'name'      => __( 'Blockons - Editor blocks', 'guten' ),
			'slug'      => 'blockons',
			'required'  => false,
		),
		array(
			'name'      => __( 'HubSpot', 'guten' ),
			'slug'      => 'leadin',
			'required'  => false,
		)
	);
	$config = array(
		...
		'strings'     => array(
			'notice_can_install_recommended'  => _n_noop(
				...
				'Guten recommends the following plugin: %1$s.',
				'Guten recommends the following plugins: %1$s.',
				'guten'
			),
			'notice_ask_to_update'            => _n_noop(
				/* translators: 1: plugin name(s). */
				'The following plugin needs to be updated to its latest version to ensure maximum compatibility with Guten: %1$s.',
				'The following plugins need to be updated to their latest version to ensure maximum compatibility with Guten: %1$s.',
				'guten'
			),
		),
	);
 ...
 */
function guten_add_admin_body_class( $guten_admin_class ) {
...
/**
 * Adjust the widget categories query if guten-blog-cats is set
 */
function guten_set_widget_categories_args($args){
	$slider_categories = get_theme_mod( 'guten-blog-cats' );
    $slider_type 	   = get_theme_mod( 'guten-slider-type', customizer_library_get_default( 'guten-slider-type' ) );
 ...
 */
function guten_add_license_notice() {
	global $pagenow;
	global $current_user;
	$guten_user_id = $current_user->ID;
	$gutenpage = isset( $_GET['page'] ) ? $pagenow . '?page=' . $_GET['page'] : $pagenow;

	if ( current_user_can( 'manage_options' ) && !get_user_meta( $guten_user_id, 'guten_admin_notice_ignore' ) ) : ?>
		<div class="notice notice-info guten-admin-notice guten-notice-add <?php echo $gutenpage != 'themes.php?page=theme_info' ? sanitize_html_class( 'guten-attract-notice' ) : ''; ?>">
			<h2>
				<?php esc_html_e( 'Thank you for using the Guten theme!', 'guten' ); ?>
			</h2>
			<p>
				<?php
				/* translators: 1: 'What Guten Premium offers' */
				printf( esc_html__( 'We\'re here to help... Please go to "Appearance -> About Guten" for help on using the theme or to see %1$s.', 'guten' ), wp_kses( __( '<a href="https://kairaweb.com/wordpress-theme/guten/#anchor-premium" target="_blank">What Guten Premium offers</a>', 'guten' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ) ); ?>
            </p>
            <p>
                <?php esc_html_e( 'Guten Premium now offers a FREE shortcode slider included, to create slides for any or all your website pages.', 'guten' ); ?>
            </p>
            <p>
                <b><?php esc_html_e( 'Use the code "SALE2021" to get $5 off Guten Pro now! Only 10 uses available this month.', 'guten' ); ?></b>
            </p><br />
			<?php if ( $gutenpage == 'themes.php?page=theme_info' ) : ?>
				<div class="guten-admin-notice-blocks">
					<div class="guten-admin-notice-block">
						<h5><?php esc_html_e( 'Popular help Links:', 'guten' ); ?></h5>
						<p>
							<?php esc_html_e( 'See our list of popular help links for building your website and/or any issues you may have.', 'guten' ); ?>
						</p>
						<ul>
							<li>
								<a href="https://kairaweb.com/wordpress-theme/guten/#anchor-premium" target="_blank"><?php esc_html_e( 'What does Guten Premium offer extra', 'guten' ); ?></a>
							</li>
							<li>
								<a href="https://kairaweb.com/documentation/setting-up-the-default-slider/" target="_blank"><?php esc_html_e( 'Setup the Guten default slider', 'guten' ); ?></a>
							</li>
							<li>
								<a href="https://kairaweb.com/documentation/adding-custom-css-to-wordpress/" target="_blank"><?php esc_html_e( 'Adding Custom CSS to WordPress', 'guten' ); ?></a>
							</li>
							<li>
								<a href="https://kairaweb.com/documentation/mobile-menu-not-working/" target="_blank"><?php esc_html_e( 'Mobile Menu is not working', 'guten' ); ?></a>
							</li>
						</ul>
					</div>
					<div class="guten-admin-notice-block">
						<h5><?php esc_html_e( 'Using Guten:', 'guten' ); ?></h5>
						<p>
							<?php esc_html_e( 'We\'ve neatly built most of the Guten settings into the WordPress Customizer so you can see all your changes happen as you built your site.', 'guten' ); ?>
						</p>
						<a href="<?php echo esc_url( admin_url( 'customize.php' ) ) ?>" class="guten-admin-notice-btn-grey">
							<?php esc_html_e( 'Start Customizing Your Website', 'guten' ); ?>
						</a>
					</div>
					<div class="guten-admin-notice-block guten-nomargin">
						<h5><?php esc_html_e( 'Need Theme Support:', 'guten' ); ?></h5>
						<p>
							<?php
							/* translators: 1: 'set up your site in WordPress', 2: 'setup WordPress Hosting' */
							printf( esc_html__( 'See our recommended %1$s and how to get ready before you start building your website after you\'ve %2$s.', 'guten' ), wp_kses( __( '<a href="https://kairaweb.com/documentation/our-recommended-wordpress-basic-setup/" target="_blank">WordPress basic setup</a>', 'guten' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ), wp_kses( __( '<a href="https://kairaweb.com/wordpress-hosting/" target="_blank">setup WordPress Hosting</a>', 'guten' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ) );
							?>
						</p>
						<a href="<?php echo esc_url( 'https://kairaweb.com/contact/' ) ?>" class="guten-admin-notice-btn-in" target="_blank">
							<?php esc_html_e( 'Get In Contact', 'guten' ); ?>
						</a>
						<a href="<?php echo esc_url( 'https://kairaweb.com/documentation/' ) ?>" class="guten-admin-notice-btn-grey" target="_blank">
							<?php esc_html_e( 'See Kaira Documentation', 'guten' ); ?>
						</a>
					</div>
				</div>
			<?php endif; ?>
			<a href="?guten_add_license_notice_ignore=" class="guten-notice-close"><?php esc_html_e( 'Dismiss Notice', 'guten' ); ?></a>
		</div><?php
	endif;
}
add_action( 'admin_notices', 'guten_add_license_notice' );
/**
 * Admin notice save dismiss to wp transient
 */
function guten_add_license_notice_ignore() {
    global $current_user;
	$guten_user_id = $current_user->ID;

    if ( isset( $_GET['guten_add_license_notice_ignore'] ) ) {
		update_user_meta( $guten_user_id, 'guten_admin_notice_ignore', true );
    }
}
add_action( 'admin_init', 'guten_add_license_notice_ignore' );

/**
 * Admin notice for Blockons
 */
function guten_blockons_notice() {
	global $current_user;
	$guten_user_id = $current_user->ID;

	if ( !get_user_meta( $guten_user_id, 'guten_blockons_dismiss' ) ) : ?>
		<div class="notice notice-info guten-admin-notice guten-notice-blockons">
			<div>
			...
			<div>
				<h4><?php esc_html_e( 'Try out the new Blockons Plugin !', 'guten' ); ?></h4>
				<p><?php esc_html_e( 'Blockons offers advanced WordPress blocks for your Editor, as well as Site Addons such as page loader, page scroll indicator & back to top button... Great for building beautiful pages! More features coming soon!', 'guten' ); ?></p>
				<a href="<?php echo esc_url(admin_url('/plugin-install.php?s=blockons&tab=search&type=term')); ?>"><?php esc_html_e( 'View the Blockons plugin', 'guten' ); ?></a>
			</div>
			<a href="?guten_blockons_notice_ignore=" class="guten-notice-close"><?php esc_html_e( 'Dismiss Notice', 'guten' ); ?></a>
		</div><?php
	endif;
}
add_action( 'admin_notices', 'guten_blockons_notice' );
/**
 * Admin notice dismiss for Blockons
 */
function guten_blockons_notice_ignore() {
    global $current_user;
	$guten_user_id = $current_user->ID;

    if ( isset( $_GET['guten_blockons_notice_ignore'] ) ) {
		update_user_meta( $guten_user_id, 'guten_blockons_dismiss', true );
    }
}
add_action( 'admin_init', 'guten_blockons_notice_ignore' );
